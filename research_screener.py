"""
research_screener.py — AlphaScope-style upside rankings (Hardcore).
"""
...
def build_rankings(snapshots: dict, macro: dict | None) -> dict:
    ...
    for sym, snap in snapshots.items():
        ...
        sent = _sentiment_score(result)
        sent_tilt = ((sent - 50) / 50 * 8.0) if sent is not None else 0.0
    ...
    n_under = sum(1 for r in rows if r["corridor_upside"] > 0)
    return {
        "ok": bool(rows),
        "rows": rows,
        "by_corridor": by_corr,
