"""
macro_engine.py — the regime layer (Hardcore). Tide under every stock move.
Light harden: logging on feed failures.
"""
...
def build_macro(client) -> dict:
    ...
    if vix_closes is not None and len(vix_closes) > 5:
        ...
        if vix_level >= 25:
            ...
            add("VIX (volatility)", vix_level, "COMPLACENT/CALM", "risk-on", LEADING)
        else:
    ...
    if rsp_closes is not None and spy_closes is not None and \
        ...
        if diff < -1.5:
            add("Breadth (RSP vs SPY, 1m)", diff, "NARROW/FRAGILE", "risk-off", LEADING)
        elif diff > 1.0:
            add("Breadth (RSP vs SPY, 1m)", diff, "BROAD/HEALTHY", "risk-on", LEADING)
        else:
    ...
    if uup_closes is not None and len(uup_closes) > 21:
        ...
        if uup_r > 0.02:
            add("US Dollar (UUP, 1m)", round(uup_r, 3), "STRENGTHENING", "risk-off", COINCIDENT)
        elif uup_r < -0.02:
            add("US Dollar (UUP, 1m)", round(uup_r, 3), "WEAKENING", "risk-on", COINCIDENT)
    ...
    lead_on = sum(1 for s in signals if s["bias"] == "risk-on" and s["lag"] == LEADING)
    lead_off = sum(1 for s in signals if s["bias"] == "risk-off" and s["lag"] == LEADING)
...
def _closes(client, symbol):
    try:
        ...
        log.debug("Macro closes for %s failed: %s", symbol, e)
        return None
