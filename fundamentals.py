"""
fundamentals.py — the trustworthy core (Hardcore edition).

Improvements:
- Cleaner rev CAGR (no always-true filter)
- pe_distribution_from_prices is the single source of truth (used by analyze)
- Better NaN handling and type safety
- Explicit logging on edge cases
"""
...
def _safe(d: dict, *keys, default=np.nan):
    """Pull first present key from an FMP record (field names drift)."""
    for k in keys:
        v = d.get(k)
...
def quality_score(income: list, balance: list, cashflow: list, ratios_ttm: dict) -> dict:
    """
    0–100 composite of business quality. Each sub-factor clamped [0,1] then weighted.
    """
    ...
    # revenue growth (CAGR). FMP returns newest first.
    rev = []
    for x in income:
        r = _safe(x, "revenue")
        if r == r and r > 0:  # not NaN and positive
            rev.append(r)
    rev_cagr = np.nan
    if len(rev) >= 2:
        # rev[0] = newest, rev[-1] = oldest
        yrs = len(rev) - 1
        try:
            rev_cagr = (rev[0] / rev[-1]) ** (1 / yrs) - 1
        except (ZeroDivisionError, ValueError):
            rev_cagr = np.nan

    factors = {
        "gross_margin":  _clamp(gross_margin / 0.6),       # 60% -> full
        "net_margin":    _clamp(net_margin / 0.25),        # 25% -> full
        "roe":           _clamp(roe / 0.25),               # 25% ROE -> full
        "liquidity":     _clamp((current_ratio - 0.8) / 1.2),
        "leverage":      _clamp(1 - (debt_to_equity / 2.0)),  # lower better
    ...
    return {
        ...
        "rev_cagr": None if rev_cagr != rev_cagr else round(float(rev_cagr), 3),
    }


def value_score(ratios_hist: list, ratios_ttm: dict) -> dict:
    """Is it cheap vs its OWN history?"""
    ...
    pe_med = float(np.median(pe_hist)) if pe_hist else np.nan
    ...
    return {
        ...
        "discount_vs_history": None if discount != discount else round(float(discount), 3),
    }
...
def simple_dcf(
    ...
    """Two-stage FCF DCF. ALL assumptions are arguments — change them and watch the answer move."""
    ...
    if discount_rate <= terminal_growth:
        log.warning("DCF discount_rate <= terminal_growth — terminal value will be huge")
    ...
    return {
        ...
        "current_price": price,
        "margin_of_safety": None if margin != margin else round(float(margin), 3),
        "assumptions": {
            "growth": growth,
...
def pe_distribution(ratios_hist: list) -> dict:
    """Median + sigma of historical P/E from ratios endpoint."""
    pes = [_safe(x, "priceEarningsRatio", "peRatio") for x in ratios_hist]
    pes = [p for p in pes if p == p and p > 0]
    if not pes:
        return {"median": None, "sigma": None, "n": 0, "source": "ratios"}
    return {
        "median": round(float(np.median(pes)), 2),
        "sigma": round(float(np.std(pes, ddof=1)) if len(pes) > 1 else 0.0, 2),
        "n": len(pes),
        "source": "ratios",
    }
...
def pe_distribution_from_prices(price_hist, earnings, lookback_days=900) -> dict:
    """Single source of truth: daily close / TTM EPS (sum of last 4 quarters).
    Used when ratios endpoint is empty or incomplete. Matches corridor.html logic.
    """
...
def ntm_eps_from_estimates(estimates: list, as_of_ms: float | None = None) -> dict:
    """Next-twelve-month EPS = sum of next 4 quarterly analyst estimates."""
    ...
    def _eps(r, which):
        return _safe(r, f"eps{which}", f"estimatedEps{which}", f"estimatedEPS{which}")
...
def _r(x, n=2):
    return None if x != x else round(float(x), n)
