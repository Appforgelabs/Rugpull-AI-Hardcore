"""
ta_engine.py — pure indicator math (no TA-Lib). Hardcore edition (identical core).
"""
...
def session_vwap(df: pd.DataFrame) -> float:
    if df is None or df.empty or "volume" not in df or df["volume"].sum() == 0:
        return float("nan")
    tp = (df["high"] + df["low"] + df["close"]) / 3
